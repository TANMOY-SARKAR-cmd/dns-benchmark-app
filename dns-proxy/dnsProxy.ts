...
export class DnsProxyServer {
  ...
  private async handleDnsQuery(msg: Buffer, rinfo: dgram.RemoteInfo): Promise<void> {
    try {
      ...
      incrementProxyStat('total', this.config.fastestProvider);
  ...
  async updateFastestProvider(domains: string[]): Promise<string> {
    ...
    // Aggregate latency by provider since the new schema drops domain from benchmark results
    ...
    Object.entries(results).forEach(([domain, domainResults]) => {
      Object.entries(domainResults).forEach(([provider, time]) => {
        if (typeof time === 'number') {
           if (!providerLatencies[provider]) providerLatencies[provider] = [];
           providerLatencies[provider].push(time);
        }
      });
    });

    // Calculate averages and log
    Object.entries(providerLatencies).forEach(([provider, times]) => {
      if (times.length > 0) {
        const avg = Math.round(times.reduce((a, b) => a + b, 0) / times.length);
        logBenchmarkResult(provider, avg);
        if (avg < fastestTime) {
          fastestTime = avg;
    ...
    try {
      import('./supabaseClient').then(({ supabase }) => {
        supabase.from('proxy_config')
          .select('id').limit(1).single().then(({data}) => {
             if (data) {
                supabase.from('proxy_config')
