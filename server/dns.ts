...
export async function testDomain(
  ...
  const promises = Object.entries(DNS_PROVIDERS).map(async ([provider, nameserver]) => {
...
export async function testDomains(
  domains: string[],
  onProgress?: (completed: number, total: number, domain: string, provider: DNSProvider) => void
