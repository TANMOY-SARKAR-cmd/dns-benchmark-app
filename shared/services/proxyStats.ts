...
export async function incrementProxyStat(
  ...
  try {
    ...
    if (type === "total")
      updatePayload.total_queries = currentStats.total_queries + 1;
    if (type === "cache_hit")
      updatePayload.cache_hits = currentStats.cache_hits + 1;
    if (type === "cache_miss")
      updatePayload.cache_misses = currentStats.cache_misses + 1;
